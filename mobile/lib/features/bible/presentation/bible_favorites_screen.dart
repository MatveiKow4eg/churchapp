...
class BibleFavoritesScreen extends ConsumerStatefulWidget {
  const BibleFavoritesScreen({super.key});

  @override
  ConsumerState<BibleFavoritesScreen> createState() => _BibleFavoritesScreenState();
}

class _BibleFavoritesScreenState extends ConsumerState<BibleFavoritesScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text(_selectionMode ? 'Выбрано: ${_selectedKeys.length}' : 'Избранное'),
        actions: [
          if (_selectionMode)
            IconButton(
              tooltip: 'Удалить',
              ...
              onPressed: () async {
                ...
                // Remove favorite flag (and delete row in DB if annotation becomes empty)
                await ref.read(bibleAnnotationsProvider.notifier).toggleFavoriteForVerses(refs);
      ...
      body: async.when(
        ...
        data: (data) {
          ...
          if (items.isEmpty) {
            return const Center(child: Text('Пока нет избранных стихов'));
          }
            ...
            builder: (context, snapshot) {
              ...
              return ListView.separated(
                ...
                itemBuilder: (context, index) {
                  ...
                  final subtitle = '$bookName ${vr.chapter}:${vr.verse}';
